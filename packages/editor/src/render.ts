...
import type { Built, ChartPattern, Point, Shape, Stitch, StitchType } from './types';
	...
	switch (s.k) {
		...
		case 'path':
			return s.fill
				...
				: `<path d="${s.d}" ${stroke}/>`;
		case 'group':
			return `<g transform="rotate(${round(s.rot)})">${s.shapes.map((x) => shapeToSVG(x, color, sw)).join('')}</g>`;
		default:
			return '';
	}
}
...
export function buildStitchShapes(
	type: StitchType,
	len?: number | null,
): Built {
...
export interface StitchSVGOpts {
	interactive?: boolean;
	color?: string;
	opacity?: number;
	klass?: string;
...
export function stitchToSVG(st: Stitch, opts: StitchSVGOpts = {}): string {
	...
	const oval =
		st.type === 'ch' ? shapes.find((s) => s.k === 'ellipse') : undefined;
	const hit =
		oval && oval.k === 'ellipse'
...
function shapesBBox(shapes: Shape[]): Box {
	...
	for (const s of shapes) {
		switch (s.k) {
			case 'line':
				expandBox(b, s.x1, s.y1);
				expandBox(b, s.x2, s.y2);
				break;
			case 'ellipse':
				expandBox(b, s.cx - s.rx, s.cy - s.ry);
				expandBox(b, s.cx + s.rx, s.cy + s.ry);
				break;
			case 'circle':
			case 'dot':
				expandBox(b, s.cx - s.r, s.cy - s.r);
				expandBox(b, s.cx + s.r, s.cy + s.r);
				break;
			case 'path': {
				const n = (s.d.match(/-?\d*\.?\d+/g) || []).map(Number);
				for (let i = 0; i + 1 < n.length; i += 2)
					expandBox(b, n[i]!, n[i + 1]!);
				break;
			}
			case 'group': {
...
function glyphViewBox(shapes: Shape[], height: number): string {
	...
	if (b.minX > b.maxX)
		b = { minX: -10, minY: -Math.max(height, 12), maxX: 10, maxY: 6 };
...
function stitchExtent(st: Placeable): Box {
	...
	if (lb.minX > lb.maxX)
		lb = { minX: -8, minY: -Math.max(height, 12), maxX: 8, maxY: 6 };
	const rot = st.rot || 0,
		sx = st.mirror ? -1 : 1;
	const out: Box = {
		minX: Infinity,
...
export function chartToSVG(pattern: ChartPattern, opts: ChartOpts = {}): string {
	const {
		padding = 30,
		background = '#ffffff',
		legend = true,
