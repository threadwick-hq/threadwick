...
import type { Base, BaseHit, Point, Stitch, StitchType } from './index';
import {
	basePoint,
	buildStitchShapes,
	chainFrom,
...
export function initCanvas(
	...
	const activeRound = (): string => {
		const p = pat();
		return p ? p.activeRound : '';
	};
	...
	function toUser(cx: number, cy: number): Point {
		const r = rect();
		return {
			x: view.panX + (cx - (r.left + r.width / 2)) / view.scale,
			y: view.panY + (cy - (r.top + r.height / 2)) / view.scale,
		};
	}
	function applyViewBox(): void {
		const r = rect();
		const w = Math.max(1, r.width) / view.scale,
			h = Math.max(1, r.height) / view.scale;
		svg.setAttribute(
			'viewBox',
			`${view.panX - w / 2} ${view.panY - h / 2} ${w} ${h}`,
		);
		const p = pat();
		if (p) p.view = { ...view };
		scheduleSave();
	...
	function scheduleSave(): void {
		...
		saveTimer = setTimeout(() => store.saveLocal(), 500);
	}
	...
	function nextStitch(): Stitch | null {
		return successorInRound(stitches(), originId, activeRound());
	}
	...
	function styleMap(): Map<string, { color?: string; opacity?: number }> {
		...
		if (originId && store.byIdMap().has(originId))
			m.set(originId, { color: ORIGIN });
		const nx = nextStitch();
		if (nx) {
	...
	function render(): void {
		...
		for (const st of stitches()) {
			...
			tiers.set(
				op,
				(tiers.get(op) || '') +
					stitchToSVG(st, { interactive: true, color: sty.color }),
			);
		}
		for (const op of [...tiers.keys()].sort((a, b) => a - b)) {
			out +=
				op < 1 ? `<g opacity="${op}">${tiers.get(op)!}</g>` : tiers.get(op)!;
		}
		out += '</g>';
		out += selectionMarkup();
		svg.innerHTML = out;
	...
	function guidesMarkup(): string {
		return (
			'<g pointer-events="none" stroke="#d9d2c4" stroke-width="1">' +
	...
	function emptyHintMarkup(): string {
		return (
			'<g pointer-events="none" opacity="0.55" text-anchor="middle" ' +
	...
	function selectionMarkup(): string {
		...
		for (const st of stitches()) {
			...
			if (origin)
				out += link(topOfStitch(origin), { x: st.x, y: st.y }, ORIGIN);
	...
	function spaceDots(emphasis: BaseHit | null): string {
		...
		for (const sp of spacesForRound(stitches(), activeRound())) {
			const isHit =
				!!emphasis &&
				emphasis.kind === 'space' &&
				emphasis.ids[0] === sp.ids[0] &&
				emphasis.ids[1] === sp.ids[1];
	...
	function drawCursor(u: Point): void {
		...
		if (effMode() !== 'insert' || drag || panning) {
			clearCursor();
			return;
		}
		const p = pat();
		if (!p || isStartRow(p, p.activeRound) || !hasStart(p)) {
			clearCursor();
			return;
		}
		...
		const bp = pendingBase
			? basePoint(store.byIdMap(), toBaseDesc(pendingBase)) || pendingBase.point
			: null;
		if (!bp) {
			cursorLayer.innerHTML = og;
			return;
		}
		const dx = u.x - bp.x,
			dy = u.y - bp.y;
		...
		cursorLayer.innerHTML =
			linkLine + og + ghostStitch(bp, len, rot) + mark(bp, SPACE, pr(4.5));
	}

	function toBaseDesc(hit: BaseHit): Base {
		return hit.kind === 'stitch'
			? { kind: 'stitch', id: hit.id }
			: { kind: 'space', ids: hit.ids };
	}
	...
	function insertDown(e: PointerEvent, u: Point): void {
		...
		if (e.altKey || e.metaKey) {
			...
			const id = hit
				...
				: (nearestStitch(stitches(), u.x, u.y, 60)?.id ?? null);
			if (id) {
				setOrigin(id);
				return;
			}
		...
		if (armed === 'ch') {
			...
			const oh = topOfStitch(o);
			const rot = (Math.atan2(u.x - oh.x, -(u.y - oh.y)) * 180) / Math.PI;
			const newId = store.placeStitch({
				type: 'ch',
				base: { kind: 'stitch', id: originId! },
				x: oh.x,
				y: oh.y,
				rot,
				len: null,
	...
	function endpointPatch(
		...
		head0: Point,
		u: Point,
	): { x?: number; y?: number; rot?: number; len?: number } {
		const aim = (x0: number, y0: number, x1: number, y1: number) =>
		...
		if (which === 'head')
			return {
				len: Math.max(2, Math.hypot(u.x - st.x, u.y - st.y)),
				rot: aim(st.x, st.y, u.x, u.y),
			};
		return {
			x: u.x,
			y: u.y,
			len: Math.max(2, Math.hypot(head0.x - u.x, head0.y - u.y)),
			rot: aim(u.x, u.y, head0.x, head0.y),
		};
	}
	...
	function selectDown(e: PointerEvent, u: Point): void {
		...
		if (hb) {
			...
			if (st) {
				endpointDrag = {
					id,
					which: hb.getAttribute('data-handle') as 'head' | 'base',
					startU: u,
					headAtStart: topOfStitch(st),
					moved: false,
				};
		...
		if (hit) {
			...
			marquee = {
				...
				cur: u,
				additive: e.shiftKey,
				base: e.shiftKey ? new Set(store.selection) : new Set(),
				moved: false,
			};
			svg.setPointerCapture(e.pointerId);
		}
	...
	function onMove(e: PointerEvent): void {
		if ((drag || panning || marquee || endpointDrag) && e.buttons === 0) {
			endGesture(e);
			return;
		}
		const u = toUser(e.clientX, e.clientY);
		...
		if (marquee) {
			...
			const x0 = Math.min(marquee.startU.x, u.x),
				y0 = Math.min(marquee.startU.y, u.y);
			const x1 = Math.max(marquee.startU.x, u.x),
				y1 = Math.max(marquee.startU.y, u.y);
			const ids = new Set(marquee.base);
			// window-pane selection: only stitches FULLY inside (head + base) make it
			for (const st of stitches())
				if (!isStart(st.type) && stitchWithinRect(st, x0, y0, x1, y1))
					ids.add(st.id);
		...
		if (endpointDrag) {
			...
			if (!endpointDrag.moved) {
				if (
					Math.hypot(u.x - endpointDrag.startU.x, u.y - endpointDrag.startU.y) *
						view.scale <
					3
				)
					return;
			...
			store.adjustStitch(
				st.id,
				endpointPatch(st, endpointDrag.which, endpointDrag.headAtStart, u),
			);
			scheduleRender();
			return;
		}
		if (drag) {
			if (!drag.moved) {
				if (
					Math.hypot(u.x - drag.startU.x, u.y - drag.startU.y) * view.scale <
					3
				)
					return;
				drag.moved = true;
			...
			const tx = u.x - drag.ox,
				ty = u.y - drag.oy;
			const lead = store.byIdMap().get(drag.leadId);
			if (lead) {
				store.dragBy(tx - lead.x, ty - lead.y);
				scheduleRender();
			} // canvas redraws; React flushes at gesture end
			return;
		}
		drawCursor(u);
	}
	...
	function onWheel(e: WheelEvent): void {
		...
		const cxPix = e.clientX - (r.left + r.width / 2),
			cyPix = e.clientY - (r.top + r.height / 2);
		const u0x = view.panX + cxPix / view.scale,
			u0y = view.panY + cyPix / view.scale;
	...
	const onLeave = () => {
		if (!marquee && !drag && !panning) clearCursor();
	};
	...
	return {
		...
		fit,
		zoomIn: () => {
			view.scale = clamp(view.scale * 1.2, 0.15, 9);
			applyViewBox();
			redrawOverlayForZoom();
		},
		zoomOut: () => {
