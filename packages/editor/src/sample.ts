...
import type { Base, Point, Project, Round, Stitch, StitchType } from './types';
...
function mk(
	...
	const dx = headPt.x - basePt.x,
		dy = headPt.y - basePt.y;
...
export function sampleProject(): Project {
	...
	prj.description =
		'A classic two-round granny square — your starter project. Open it to see how every stitch comes out of an origin and is worked into a base.';
	const pat = newPattern('Classic granny square');
	const r0: Round = { id: uid('rnd'), name: 'Start' };
	const r1 = pat.rounds[0]!;
	r1.name = 'Round 1';
	...
	const edges = [0, 90, 180, 270],
		corners = [45, 135, 225, 315],
		R1 = 48;
	const seq1: { type: StitchType; ang: number; radius: number }[] = [];
	for (let k = 0; k < 4; k++) {
		for (const off of [-12, 0, 12])
			seq1.push({ type: 'dc', ang: edges[k]! + off, radius: R1 });
		seq1.push({ type: 'ch', ang: corners[k]!, radius: R1 * 0.96 });
	}
	for (const it of seq1) {
		if (it.type === 'ch') {
			...
			const s = mk(
				...
				{ kind: 'stitch', id: prev },
				oh,
				fromPolar(it.radius + 16, it.ang),
			);
			stitches.push(s);
			prev = s.id;
		} else {
			const head = fromPolar(it.radius, it.ang);
			const s = mk(
				r1.id,
				it.type,
				prev,
				{ kind: 'stitch', id: ring.id },
				{ x: 0, y: 0 },
				head,
			);
			stitches.push(s);
	...
	const real1 = stitches.filter(
		(s) => s.round === r1.id && isRealStitch(s.type),
	);
	const nearestBaseRef = (pt: Point): Base => {
		let best: Stitch = ring,
			bd = Infinity;
		for (const s of real1) {
	...
	const Rin = 52;
	const items2: { type: StitchType; ang: number; radius: number }[] = [];
	for (let k = 0; k < 4; k++) {
		const c = corners[k]!;
		items2.push(
			{ type: 'dc', ang: c - 13, radius: 112 },
			{ type: 'dc', ang: c - 6, radius: 116 },
			{ type: 'ch', ang: c, radius: 120 },
			{ type: 'dc', ang: c + 6, radius: 116 },
			{ type: 'dc', ang: c + 13, radius: 112 },
		);
		const e = edges[k]!;
		items2.push(
			{ type: 'dc', ang: e - 9, radius: 92 },
			{ type: 'dc', ang: e, radius: 92 },
			{ type: 'dc', ang: e + 9, radius: 92 },
		);
	}
