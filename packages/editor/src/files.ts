...
export function exportPatternSVG(
	...
	download(
		`${slug(pattern.name, 'pattern')}.svg`,
		new Blob([chartSVG(pattern, opts)], { type: 'image/svg+xml' }),
	);
}
...
export function exportPatternPNG(
	...
	img.onload = () => {
		...
		URL.revokeObjectURL(url);
		canvas.toBlob((b) => {
			if (b) download(`${slug(pattern.name, 'pattern')}.png`, b);
		}, 'image/png');
	};
		...
		opts.instructions === false
			...
			: pat.rounds
					.map((r) => {
						...
						return text
							? `<li><b>${escapeXML(r.name)}:</b> ${escapeXML(text)}</li>`
							: '';
					})
					.filter(Boolean)
					.join('');
...
function printDoc(title: string, bodyInner: string): string {
	return (
		`<!doctype html><html><head><meta charset="utf-8"><title>${escapeXML(title)}</title>` +
...
export function printPattern(
	...
	if (!win) return;
	const heading =
		opts.title === false ? '' : `<h1>${escapeXML(pattern.name)}</h1>`;
	writeAndPrint(
		win,
		printDoc(
			pattern.name,
			`${heading}${patternSection(pattern, { ...opts, title: false })}`,
		),
	);
}
...
async function qrSvg(text: string): Promise<string> {
	try {
		return await QRCode.toString(text, { type: 'svg', margin: 1 });
	} catch {
		return '';
	}
}
...
export async function printProject(project: Project): Promise<void> {
	...
	win.document.write(
		'<!doctype html><meta charset="utf-8"><body style="font:16px Georgia,serif;color:#777;padding:28px">Preparing printable PDF…</body>',
	);

	const ver = activeVersion(project);
	const patterns =
		ver.patterns.map((p) => patternSection(p, {})).join('') ||
		'<p class="muted">This project has no patterns yet.</p>';

	const r = ver.resources;
	const block = (heading: string, items: string[]): string =>
		items.length
			? `<h3>${escapeXML(heading)}</h3><ul>${items.join('')}</ul>`
			: '';
	const yarns = block(
		'Yarns',
		r.yarns.map(
	...
	const notes = block(
		'Notes & tips',
		r.notes.map(
			(n) => `<li><b>${escapeXML(n.title)}</b> ${escapeXML(n.body)}</li>`,
		),
	);
	const variations = block(
		'Variations',
		r.variations.map(
			(v) => `<li><b>${escapeXML(v.title)}</b> ${escapeXML(v.body)}</li>`,
		),
	);
	...
	const linkItems = await Promise.all(
		r.links.map(async (l) => {
			...
			return (
				`<li>${qr ? `<div class="qr">${qr}</div>` : ''}<div class="link-text"><b>${escapeXML(l.title || l.url || 'Link')}</b>` +
				`${l.url ? `<span class="url">${escapeXML(l.url)}</span>` : ''}</div></li>`
			);
		}),
	);
	const links = r.links.length
		? `<h3>Links &amp; videos</h3><ul class="links">${linkItems.join('')}</ul>`
		: '';

	const resources =
		yarns || links || notes || variations
	...
	const body =
		...
		(project.description
			? `<p class="desc">${escapeXML(project.description)}</p>`
			: '') +
		patterns +
		resources;
