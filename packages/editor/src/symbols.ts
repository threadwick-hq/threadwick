...
import type { Built, Shape, StitchType } from './types';

export const BAR = 15; // width of the horizontal "head" bar on post stitches

export function postShapes(
	height: number,
	slashes = 0,
	{ topBar = true }: { topBar?: boolean } = {},
): Shape[] {
...
export const STITCHES: Record<StitchType, StitchDef> = {
	ch: {
		...
		build: () => {
			const B = 6,
				...
				ry = 5;
			return {
				shapes: [{ k: 'ellipse', cx: 0, cy: -(B + ry), rx, ry }],
				height: B + 2 * ry,
				head: { x: 0, y: -(B + 2 * ry) },
			};
		},
	...
	mr: {
		...
		kind: 'start',
		build: () => ({
			shapes: [{ k: 'circle', cx: 0, cy: 0, r: 15 }],
			height: 0,
		}),
	},
	dmr: {
		...
		build: () => ({
			shapes: [
				{ k: 'circle', cx: 0, cy: 0, r: 15 },
				{ k: 'circle', cx: 0, cy: 0, r: 11 },
			],
			height: 0,
		}),
	},
		...
		build: () => {
			...
			for (let i = 0; i < N; i++)
				shapes.push({
					k: 'group',
					rot: (i * 360) / N,
					shapes: [{ k: 'ellipse', cx: R, cy: 0, rx: 2.7, ry: 4.6 }],
				});
...
export const STITCH_ORDER: StitchType[] = [
	'ch',
	'slst',
	'sc',
	'hdc',
...
export const STITCH_KEYS: Partial<Record<StitchType, string>> = {
	ch: 'c',
	slst: 'l',
	sc: 's',
	hdc: 'h',
