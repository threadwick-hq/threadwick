...
class Store {
	...
	publishVersion(projectId: string): void {
		...
		if (!draft) return;
		for (const v of prj.versions)
			if (v.status === 'published') v.status = 'outdated';
		draft.status = 'published';
		draft.publishedAt = nowISO();
		draft.updatedAt = nowISO();
		prj.activeVersionId = draft.id;
	...
	discardDraft(projectId: string): void {
		...
		prj.activeVersionId = (
			publishedVersion(prj) ?? prj.versions[prj.versions.length - 1]!
		).id;
		prj.updatedAt = nowISO();
		this.clearEditor();
		if (this.state.ui.view === 'editor')
	...
	addResource<K extends ResourceKind>(
		projectId: string,
		kind: K,
		item: Partial<Omit<Resources[K][number], 'id'>>,
	): string | null {
		const prj = this.getProject(projectId);
		if (!prj) return null;
		const ver = activeVersion(prj);
	...
	updateResource<K extends ResourceKind>(
		projectId: string,
		kind: K,
		itemId: string,
		patch: Partial<Omit<Resources[K][number], 'id'>>,
	): void {
		const prj = this.getProject(projectId);
		if (!prj) return;
		const ver = activeVersion(prj);
		if (ver.status !== 'draft') return;
		const it = (ver.resources[kind] as Resources[K][number][]).find(
			(x) => x.id === itemId,
		);
		if (it) {
	...
	editTransact(fn: (pat: Pattern) => void): void {
		...
		if (ver) ver.updatedAt = nowISO();
		const prj = this.currentProject();
		if (prj) prj.updatedAt = nowISO();
	...
	removeRound(roundId: string): void {
		this.editTransact((pat) => {
			...
			const removed = new Set(
				pat.stitches.filter((s) => s.round === roundId).map((s) => s.id),
			);
			...
			for (const s of pat.stitches) {
				...
				if (s.base && s.base.kind === 'stitch' && removed.has(s.base.id))
					s.base = null;
				if (
					s.base &&
					s.base.kind === 'space' &&
					(removed.has(s.base.ids[0]) || removed.has(s.base.ids[1]))
				)
					s.base = null;
			}
			if (
				!pat.rounds.find((r) => r.id === pat.activeRound) ||
				isStartRow(pat, pat.activeRound)
			) {
				const firstWorking = pat.rounds.find((r) => !isStartRow(pat, r.id));
				pat.activeRound = (
					firstWorking || pat.rounds[pat.rounds.length - 1]!
				).id;
			}
		});
	}
	...
	placeStitch(params: PlaceParams): string | null {
		...
		this.editTransact((pat) => {
			...
			id = uid('st');
			if (originId) {
				const next = pat.stitches.find(
					(s) => s.round === roundId && s.origin === originId,
				);
	...
	adjustStitch(
		id: string,
		patch: { x?: number; y?: number; rot?: number; len?: number },
	): void {
	...
	removeStitches(ids: string[]): void {
		...
		this.editTransact((pat) => {
			...
			const resolve = (originId: string | null): string | null => {
				let cur = originId;
			...
			for (const s of pat.stitches) {
				...
				if (
					s.base &&
					s.base.kind === 'space' &&
					(set.has(s.base.ids[0]) || set.has(s.base.ids[1]))
				)
					s.base = null;
			}
		});
	...
	evenRound(roundId: string): void {
		this.editTransact((pat) => {
			const order = chainOrder(pat.stitches, roundId).filter((s) =>
				isRealStitch(s.type),
			);
			...
			cy /= order.length;
			let R = 0;
			const heads = order.map((s) => topOfStitch(s));
...
function autoLayoutChains(pat: Pattern): void {
	...
	const ovalCy =
		built.shapes[0] && built.shapes[0].k === 'ellipse' ? built.shapes[0].cy : 0;
	const d0 = -ovalCy;
	for (const s of stitches) {
		if (s.type !== 'ch' || s.auto === false) continue;
		let a = s.origin ? byId.get(s.origin) : undefined,
			before = 0;
		const seenA = new Set([s.id]);
		while (a && a.type === 'ch' && !seenA.has(a.id)) {
			before++;
			seenA.add(a.id);
			a = a.origin ? byId.get(a.origin) : undefined;
		}
		let c = childOf.get(s.id),
			after = 0;
		const seenC = new Set([s.id]);
		while (c && c.type === 'ch' && !seenC.has(c.id)) {
			after++;
			seenC.add(c.id);
			c = childOf.get(c.id);
		}
		if (!a || !c || a.type === 'ch' || c.type === 'ch') continue;
		const ah = topOfStitch(a),
			chd = topOfStitch(c);
		const dx = chd.x - ah.x,
			dy = chd.y - ah.y;
