...
import type { Base, BaseHit, Point, Round, SpaceSlot, Stitch } from './types';
...
export function successorInRound(
	stitches: Stitch[],
	...
	return (
		stitchesInRound(stitches, roundId).find((s) => s.origin === originId) ??
		null
	);
}
...
export function spacesForRound(
	stitches: Stitch[],
	roundId: string,
): SpaceSlot[] {
	const real = chainOrder(stitches, roundId).filter((s) =>
		isRealStitch(s.type),
	);
	const out: SpaceSlot[] = [];
	for (let i = 0; i + 1 < real.length; i++) {
		const a = real[i]!,
			b = real[i + 1]!;
		const ta = topOfStitch(a),
			tb = topOfStitch(b);
		out.push({
			ids: [a.id, b.id],
			point: { x: (ta.x + tb.x) / 2, y: (ta.y + tb.y) / 2 },
		});
	}
	return out;
}
...
export function basePoint(byId: Map<string, Stitch>, base: Base): Point | null {
	...
	if (base.kind === 'stitch') {
		...
		return s ? topOfStitch(s) : null;
	}
	const a = byId.get(base.ids[0]),
		b = byId.get(base.ids[1]);
	if (!a || !b) return null;
	const ta = topOfStitch(a),
		tb = topOfStitch(b);
...
export function pickBase(
	...
	x: number,
	y: number,
	opts: { maxD?: number; exclude?: Set<string> | null } = {},
): BaseHit | null {
	const maxD = opts.maxD ?? 82,
	...
	for (const s of stitches) {
		...
		if (d < bd) {
			bd = d;
			best = { kind: 'stitch', id: s.id, point: pt, d };
		}
	}
...
export function stitchWithinRect(
	st: Stitch,
	x0: number,
	y0: number,
	x1: number,
...
export function nearestStitch(
	stitches: Stitch[],
	x: number,
	y: number,
	maxD = Infinity,
): Stitch | null {
	let best: Stitch | null = null,
		bd = maxD;
	for (const s of stitches) {
		const pt = topOfStitch(s);
		const d = Math.hypot(pt.x - x, pt.y - y);
...
export function defaultOriginId(
	stitches: Stitch[],
	rounds: Round[],
