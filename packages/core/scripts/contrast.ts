...
export function oklchToLinearSrgb([Lp, C, H]: Triple): [
	number,
	number,
	number,
] {
...
export const PAIRS: Pair[] = [
	...
	{ fg: 'focus', bg: 'bgLayout', min: 3, note: 'focus ring on page (1.4.11)' },
	{
		...
		bg: 'bgContainer',
		min: 3,
		note: 'focus ring on surface (1.4.11)',
	},
];
...
export function auditContrast(tokensJsonPath: string): ContrastRow[] {
	// biome-ignore lint/suspicious/noExplicitAny: token JSON is a loosely-typed DTCG tree parsed at audit time.
	const dt: any = JSON.parse(readFileSync(tokensJsonPath, 'utf8'));
	const role = (name: string) => dt.color.role[name];
	// biome-ignore lint/suspicious/noExplicitAny: DTCG token nodes are untyped at the parse boundary.
	const triple = (tok: any, mode: Mode): Triple =>
		mode === 'dark'
			? (tok.$extensions[TW].dark?.oklch ?? tok.$extensions[TW].oklch)
			: tok.$extensions[TW].oklch;
