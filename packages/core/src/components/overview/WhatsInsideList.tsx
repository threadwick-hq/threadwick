...
export function WhatsInsideList({ items, linkComponent, className }: WhatsInsideListProps) {
				...
				{items.map((item) => (
					<li key={item.id}>
						{linkComponent({
							to: item.href,
							className:
								'flex items-center gap-3 rounded-md border border-border px-3 py-2.5 transition-colors hover:bg-muted/50',
							children: (
								<>
									<Icon
										name={item.icon}
