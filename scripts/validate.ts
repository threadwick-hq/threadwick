...
const ALLOWED_PX = new Set([0, 1, 2, 4, 6, 8, 10, 12, 16, 20, 24, 32, 40, 44, 48, 64, 80, 96, 264, 1120]);
...
for (const file of files) {
  ...
  lines.forEach((line, i) => {
    ...
    for (const h of line.match(/#[0-9a-fA-F]{3,8}\b/g) ?? []) {
      violations.push({ file, line: ln, rule: 'non-token-colour', detail: `raw hex ${h} — use a --tw-* token` });
    }
    if (isCss && SPACING_PROP.test(line)) {
      for (const px of line.match(/\d+px/g) ?? []) {
        const n = parseInt(px, 10);
        if (!ALLOWED_PX.has(n)) {
          violations.push({ file, line: ln, rule: 'off-grid', detail: `${px} is off the 8-px grid — use a space token` });
        }
      }
    }
  });
  ...
  while ((m = re.exec(content)) !== null) {
    ...
    if (!/aria-label|aria-labelledby|aria-hidden|<title/.test(win)) {
      ...
      violations.push({ file, line: ln, rule: 'missing-aria', detail: 'role="img" without an accessible name (aria-label / aria-labelledby / <title>)' });
    }
  }
}
...
if (flags.has('--json')) {
  console.log(JSON.stringify({ scanned: files.length, targets, violations }, null, 2));
} else {
  console.log(`validate: scanned ${files.length} file(s) in ${targets.join(', ')} + token contrast`);
  for (const v of violations) console.log(`  ${v.file}:${v.line}  [${v.rule}] ${v.detail}`);
  console.log(violations.length === 0 ? '  ✓ no violations' : `  ✗ ${violations.length} violation(s)`);
}
