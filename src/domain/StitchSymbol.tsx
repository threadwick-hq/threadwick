...
import { STITCH_SLASHES, type StitchType } from './stitches';
 ...
 * (StitchSymbol) and composed into larger chart artwork.
 *
 * Colour defaults to the themed text token (`var(--tw-text)`), so the glyph follows
...
/**
 * A standalone, labelled symbol — used by the legend. When `label` is provided it is an
 * accessible image (`role="img"` + `aria-label`); without one it is decorative (`aria-hidden`).
 */
